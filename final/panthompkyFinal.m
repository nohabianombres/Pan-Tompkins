...
mat_filename = '114m.mat';  % Actualiza con el nombre del archivo que deseas cargar
...
if exist(mat_fullpath, 'file') == 2
    ...
    fre_car = mean(QRSs)/Fs
    subplot(7, 1, 7);
    plot(electro_delayed);
    hold on;
    plot(QRSs, electro_delayed(QRSs), 'ro');
    title('g) Señal de ECG con Detección de QRS, Fecuencia cardiaca' );
    xlabel('Muestras');
    ylabel('Amplitud');
    legend('ECG', 'QRS');
